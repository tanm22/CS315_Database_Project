    ...
    try {
        ...
        if ($delete_id <= 0) {
            ...
            if ($check_order->rowCount() === 0) {
                ...
                $delete_order_items = $conn->prepare("DELETE FROM `order_items` WHERE order_id = ?");
      ...
      try {
          ...
          if ($select_orders->rowCount() > 0) {
              while ($fetch_orders = $select_orders->fetch(PDO::FETCH_ASSOC)) {
                  ...
                  $select_items = $conn->prepare("SELECT SUM(quantity) as total_products FROM `order_items` WHERE order_id = ?");
                  $select_items->execute([$order_id]);
                  $total_products = $select_items->fetchColumn() ?: 0;
   ?>
   <div class="box">

   ...
   try {
      ...
      if (empty($name) || strlen($name) > 20) {
         ...
         if ($check_cart->rowCount() > 0) {
            ...
            $insert_item = $conn->prepare("INSERT INTO `order_items`(order_id, pid, quantity, name, price) 
                                           VALUES(?,?,?,?,?)");
            foreach ($cart_items as $item) {
               $insert_item->execute([$order_id, $item['pid'], $item['quantity'], $item['name'], $item['price']]);
            }

...
if (isset($_GET['delete'])) {
    try {
        ...
        // Delete related data
        $delete_order_items = $conn->prepare("DELETE FROM `order_items` WHERE order_id IN (SELECT id FROM `orders` WHERE user_id = ?)");
        ...
        // Delete users_contact if no other users have the same email
        $check_other_users = $conn->prepare("SELECT COUNT(*) FROM `users` WHERE email = ?");
        $check_other_users->execute([$email]);
        $count = $check_other_users->fetchColumn();
        if ($count == 0) {
            $delete_contact = $conn->prepare("DELETE FROM `users_contact` WHERE email = ?");
            $delete_contact->execute([$email]);
        }

    ...
    if ($user_id) {
        try {
            ...
            if (empty($review)) {
                ...
                $check_purchase = $conn->prepare("SELECT * FROM `orders` WHERE user_id = ? AND total_products LIKE ?");
                $check_purchase->execute([$user_id, '%'. $product_id . ' (x%']);
...
<section class="quick-view">
    ...
    <?php if ($user_id) {
        // Check if user purchased the product and hasn't reviewed it
        $check_purchase = $conn->prepare("SELECT * FROM `orders` WHERE user_id = ? AND total_products LIKE ?");
        $check_purchase->execute([$user_id, '%'. $pid . ' (x%']);
        $check_review = $conn->prepare("SELECT user_id, pid FROM `reviews` WHERE user_id = ? AND pid = ?");
        $check_review->execute([$user_id, $pid]);
        if ($check_purchase->rowCount() > 0 && $check_review->rowCount() == 0) {
    ?>
    <div>
    <h2 style="padding-left: 4rem;">Submit Your Review</h2>
    ...
    <?php
            ...
            echo '<p class="empty">Purchase this product to submit a review!</p>';
        } else {

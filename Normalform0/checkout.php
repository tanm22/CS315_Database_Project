...
if (isset($_POST['order'])) {
   try {
      ...
      if (empty($name) || strlen($name) > 20) {
         ...
         $check_cart = $conn->prepare("SELECT pid, quantity, name, price FROM `cart` WHERE user_id = ?");
         ...
         if ($check_cart->rowCount() > 0) {
            ...
            $total_products = '';
            foreach ($cart_items as $item) {
               $total_products .= $item['pid'] . ' (x' . $item['quantity'] . '), ';
            }
            $total_products = rtrim($total_products, ', ');

            $insert_order = $conn->prepare("INSERT INTO `orders`(user_id, name, number, email, method, address, total_price, total_products) 
                                    VALUES(?,?,?,?,?,?,?,?)");
            $insert_order->execute([$user_id, $name, $number, $email, $method, $address, $grand_total, $total_products]);
...
<section class="checkout-orders">
   <form action="" method="POST">
      ...
      $grand_total = 0;
      try {
         $select_cart = $conn->prepare("SELECT quantity, name, price FROM `cart` WHERE user_id = ?");
